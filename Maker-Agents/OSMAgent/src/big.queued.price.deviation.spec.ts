...
describe("Big deviation queued price Tests", () => {
  ...
  it("should return a finding when the new price deviates too much", async () => {
    const log = logIface.encodeEventLog(logIface.getEvent("LogValue"), [abiCoder.encode(["uint256"], [100])]);

    const txEvent: TransactionEvent = new TestTransactionEvent()
      .addTraces({
        from: CONTRACTS.get("PIP_ONE"),
        input: PEEK_FUNCTION_SELECTOR,
        output: PEEK_ABI.encodeFunctionResult("peek", [abiCoder.encode(["uint256"], [107]), true]),
      })
  ...
  it("should return an empty finding if the new price doesn't deviate too much", async () => {
    ...
    const txEvent: TransactionEvent = new TestTransactionEvent()
      .addTraces({
        from: CONTRACTS.get("PIP_ONE"),
        input: PEEK_FUNCTION_SELECTOR,
        output: PEEK_ABI.encodeFunctionResult("peek", [abiCoder.encode(["uint256"], [105]), true]),
      })
      .addAnonymousEventLog(CONTRACTS.get("PIP_ONE"), log.data, ...log.topics);
  ...
  it("should return an empty finding if the Peek function wasn't called from the correct contract", async () => {
    ...
    const txEvent: TransactionEvent = new TestTransactionEvent()
      .addTraces({
        from: ADDRESSES[1],
        input: PEEK_FUNCTION_SELECTOR,
        output: PEEK_ABI.encodeFunctionResult("peek", [abiCoder.encode(["uint256"], [200]), true]),
      })
      .addAnonymousEventLog(CONTRACTS.get("PIP_ONE"), log.data, ...log.topics);
  ...
  it("should return an empty finding if Peek function wasn't called", async () => {
    const txEvent: TransactionEvent = new TestTransactionEvent().addTraces({
      from: CONTRACTS.get("PIP_ONE"),
      input: "0x11111111",
      output: PEEK_ABI.encodeFunctionResult("peek", [abiCoder.encode(["uint256"], [108]), true]),
    });
  ...
  it("should return an empty finding if the Peek call wasn't successful", async () => {
    const txEvent: TransactionEvent = new TestTransactionEvent().addTraces({
      from: CONTRACTS.get("PIP_ONE"),
      input: PEEK_FUNCTION_SELECTOR,
      output: PEEK_ABI.encodeFunctionResult("peek", [abiCoder.encode(["uint256"], [108]), false]),
    });
  ...
  it("should return an empty finding if the event with deviated price was not emitted from the correct address", async () => {
    const log1 = logIface.encodeEventLog(logIface.getEvent("LogValue"), [abiCoder.encode(["uint256"], [100])]);
    const log2 = logIface.encodeEventLog(logIface.getEvent("LogValue"), [abiCoder.encode(["uint256"], [105])]);
  ...
  it("should return multiple finding if there are multiple Oracles with big deviations in new prices", async () => {
    const log1 = logIface.encodeEventLog(logIface.getEvent("LogValue"), [abiCoder.encode(["uint256"], [100])]);
    const log2 = logIface.encodeEventLog(logIface.getEvent("LogValue"), [abiCoder.encode(["uint256"], [90])]);
    ...
    const txEvent: TransactionEvent = new TestTransactionEvent()
      ...
      .addTraces({
        ...
        input: PEEK_FUNCTION_SELECTOR,
        output: PEEK_ABI.encodeFunctionResult("peek", [abiCoder.encode(["uint256"], [118]), true]),
      })
  ...
  it("should only return findings from the Oracles with big deviations on new prices", async () => {
    const log1 = logIface.encodeEventLog(logIface.getEvent("LogValue"), [abiCoder.encode(["uint256"], [100])]);
    const log2 = logIface.encodeEventLog(logIface.getEvent("LogValue"), [abiCoder.encode(["uint256"], [90])]);
    const log3 = logIface.encodeEventLog(logIface.getEvent("LogValue"), [abiCoder.encode(["uint256"], [100])]);
    const txEvent: TransactionEvent = new TestTransactionEvent()
      .addTraces({
        from: CONTRACTS.get("PIP_ONE"),
        input: PEEK_FUNCTION_SELECTOR,
      ...
      .addTraces({
        ...
        input: PEEK_FUNCTION_SELECTOR,
        output: PEEK_ABI.encodeFunctionResult("peek", [abiCoder.encode(["uint256"], [101]), false]),
      });
