...
const getFindingsForAJar = async (pickleJarAddress: string, blockNumber: number, provider: providers.Provider): Promise<Finding[]> => {
  ...
  const availablePromise = getPickleJarsAvailable(pickleJarAddress, blockNumber, provider);
...
export const providerHandleBlock = (pickleRegistryAddress: string, provider: providers.Provider): HandleBlock => {
  return async (blockEvent: BlockEvent): Promise<Finding[]> => {
    const pickleJars = await getPickleJars(pickleRegistryAddress, blockEvent.blockNumber, provider);
    
    const findingsPromises = pickleJars.map((pickleJar: string) => getFindingsForAJar(pickleJar, blockEvent.blockNumber, provider));
...
export default {
  handleBlock: providerHandleBlock(PICKLE_FINANCE_REGISTRY, getEthersProvider()),
};
