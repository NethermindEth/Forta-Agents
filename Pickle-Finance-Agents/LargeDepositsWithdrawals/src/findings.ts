...
const WhitdrawFinding: FindingGenerator = (log: LogDescription): Finding => Finding.fromObject({
...
export const createFinding: FindingGenerator = (log: LogDescription): Finding =>
  BigNumber.from(0).eq(log.args[0])? DepositFinding(log) : WhitdrawFinding(log);
