...
describe("VaultsFetcher tests suite", () => {
  ...
  it("should return the correct values", async () => {
    ...
    for(let [vault, block, ratio] of TEST_DATA){
      const value: BigNumber = await fetcher.getSupply(block, vault);
  ...
  it("should return all the vaults", async () => {
    ...
    // get the vaults
    let vaults: Set<string> = await fetcher.getVaults(500);
    for(let vault of ALL)
      expect(vaults.has(vault)).toStrictEqual(true);
    expect(vaults.size).toStrictEqual(ALL.length);
