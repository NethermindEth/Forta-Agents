...
describe("Benqi Finance Large Pool Withdrawal Agent Test Suite", () => {
  ...
  beforeAll(() => {
    // Create a set (which would normally be created by the agents `initialize` function
    initializeSet = new Set<string>();
    initializeSet.add(QITOKEN_ADDR);
    // Setup the mock function to get total pool supply to be used by the agent
    mockGetTotalSupply = jest.fn();
    // Setup the handler
    handler = provideHandleTransaction(initializeSet, mockGetTotalSupply, 0.25, COMPTROLLER_ADDR);
  });
  ...
  it("updates the set of qitokens when a new market is listed", async () => {
    ...
    // Convert the set to an array to access the last element
    const initializeSetArray = Array.from(initializeSet);

    // The new QiToken should have been added to the QiToken set
    expect(initializeSetArray[1]).toStrictEqual(NEW_QITOKEN_ADDR);
  });
});
