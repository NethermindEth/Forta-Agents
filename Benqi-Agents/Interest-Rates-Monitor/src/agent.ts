...
export const provideHandleBlock =
  ...
  async (blockEvent: BlockEvent) => {
    ...
    const qiTokenSupplyInterestRate: BigNumber[] = await Promise.all(
      marketsFetcher.markets.map((qiToken) => fetcher.getSupplyInterestRates(block, qiToken))
    );
    const qiTokenBorrowInterestRate: BigNumber[] = await Promise.all(
      marketsFetcher.markets.map((qiToken) => fetcher.getBorrowInterestRates(block, qiToken))
    );
    ...
    qiTokenSupplyInterestRate.forEach((supplyRate, i) => {
      ...
      if (supplyRate.lt(thresholds.supply[0])) {
        findings.push(createFinding(marketsFetcher.markets[i], supplyRate, thresholds.supply[0], findingCase[0]));
      } //Upper supply threshold check
      else if (supplyRate.gt(thresholds.supply[1])) {
        findings.push(createFinding(marketsFetcher.markets[i], supplyRate, thresholds.supply[1], findingCase[1]));
      }
    });
    ...
    qiTokenBorrowInterestRate.forEach((borrowRate, i) => {
      ...
      if (borrowRate.lt(thresholds.borrow[0])) {
        findings.push(createFinding(marketsFetcher.markets[i], borrowRate, thresholds.borrow[0], findingCase[2]));
      } //Upper borrow threshold check
      else if (borrowRate.gt(thresholds.borrow[1])) {
        findings.push(createFinding(marketsFetcher.markets[i], borrowRate, thresholds.borrow[1], findingCase[3]));
      }
    });
