...
// Format:[network, multicall2, factory, pairInitCodeHash][];
const TEST_CASES: [number, string, Record<number, string>, string, string][] = [
  [
    11,
    createAddress("0xa2a"),
...
const generateNetworkMap = (
  ...
  networkMap[data[0]] = {
    chainId: data[0],
    multicall2: data[1],
    multicall2Data: data[2],
    factory: data[3],
    pairInitCodeHash: data[4],
  } as NetworkData;
...
describe("NetworkManager test suite", () => {
  it("should return the correct contract address for each network", async () => {
    for (let testCase of TEST_CASES) {
      ...
      const [network, multicall2, multicall2Data, factory, pairInitCodeHash] = testCase;
