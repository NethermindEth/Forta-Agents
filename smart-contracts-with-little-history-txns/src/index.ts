...
export const ETHERSCAN_API_ENDPOINT: string =
  "https://api.etherscan.io/api?module=account&action=txlistinternal&sort=asc";
...
function provideHandleTransaction(getter: any): HandleTransaction {
  return async function handleTransaction(
    ...
    if (data.result.length < HISTORY_THRESHOLD) {
      findings.push(
        Finding.fromObject({
          name: "Transaction Detection on Smart Contract with Little History",
          description: `Contract transaction detected with little history.`,
          alertId: "NETHFORTA-7",
          type: FindingType.Suspicious,
          severity: FindingSeverity.Medium,
          metadata: {
            contract: address,
            length: data.result.length.toString()
          }
        })
      );
    }
