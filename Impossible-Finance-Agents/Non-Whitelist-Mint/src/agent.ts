...
import { Verifier, IF_WHITELIST_VERIFIER, IDIA_WHITELIST_VERIFIER } from "./verifier";
...
const TOKENS: [string, Verifier, string, string][] = [
  [IF_ADDR, IF_WHITELIST_VERIFIER, "IF", "IMPOSSIBLE-2-1"],
  [IDIA_ADDR, IDIA_WHITELIST_VERIFIER, "IDIA", "IMPOSSBILE-2-2"],
];
          ...
          await Promise.all(
            transfers.map(async (transfer) => {
              ...
              if (transfer.args.from == ZERO_ADDR) {
                ...
                const whitelisted = await verifierFunction(transfer.args.to);
