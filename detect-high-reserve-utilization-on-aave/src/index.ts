...
const reserveUtilizationGetter: ReserveUtilizationGetter =
  new ReserveUtilizationGetter(web3);
...
export const provideHandleBlock = (
  ...
  return async (blockEvent: BlockEvent) => {
    ...
    for (let asset in Object.values(Assets)) {
      ...
      const utilization = await reserveUtilizationGetter.getUtilization(
        assetAddress
      );
