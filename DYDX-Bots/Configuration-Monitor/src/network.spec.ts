...
describe("NetworkManager test suite", () => {
  it("should return the correct contract address for each network", async () => {
    for (let data of TEST_ADDRESSES) {
      const networkMap: Record<number, NetworkData> = generateNetworkMap(TEST_NETWORKS, data);
      const networkMan = new NetworkManager(networkMap);
      for (let i = 0; i < TEST_NETWORKS.length; i++) {
        networkMan.setNetwork(TEST_NETWORKS[i]);
        expect(networkMan.perpetualProxy).toStrictEqual(data[i]);
      }
      // test with a non supported network
      networkMan.setNetwork(99);
      expect(networkMan.perpetualProxy).toStrictEqual("");
    }
  });
});
