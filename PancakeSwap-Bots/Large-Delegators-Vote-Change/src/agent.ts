...
const provideHandleTransaction = (networkManager: NetworkManager<NetworkData>): HandleTransaction => {
  return async (txEvent: TransactionEvent): Promise<Finding[]> => {
    ...
    delegateVotesChangedEvents.forEach((delegateVotesChangedEvent) => {
      ...
      if (ABSOLUTE_THRESHOLD.lte(newBalance.toString())) {
        findings.push(createFinding(delta.toString(), metadata, FindingSeverity.Critical));
      } else if (MIN_PREVIOUS_BALANCE.lt(previousBalance.toString())) {
        let deltaPercentage: BN = delta.dividedBy(new BN(previousBalance.toString())); //percentage of vote increase
