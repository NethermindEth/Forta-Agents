...
export const createAddr = (addr: string): string => Web3.utils.leftPad(addr, 40);

export const createEncodedAddr = (addr: string): string => _web3.eth.abi.encodeParameter("address", createAddr(addr));

export const createEncodedUint256 = (value: BigNumber): string => _web3.eth.abi.encodeParameter("uint256", value);

export const hatCall = (): string => _web3.eth.abi.encodeFunctionCall(HAT_JSON_INTERFACE, []);
...
export const propertyFetcher =
  (web3Call: any, address: string, dataEncoder: any, propertyType: string): PropertyFetcher =>
