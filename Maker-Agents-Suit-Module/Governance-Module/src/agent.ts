import { BlockEvent, Finding, getJsonRpcUrl, HandleBlock, HandleTransaction } from "forta-agent";
...
const spellsManager: AddressManager = new DeployedAddressesManager(SPELL_DEPLOYER, _web3.eth.getTransactionCount);
...
export function provideHandleTransaction(addressManager: AddressManager): HandleTransaction {
  return provideLiftEventsListener("MakerDAO-GM-2", CHIEF_CONTRACT, addressManager.isKnownAddress.bind(addressManager));
}
