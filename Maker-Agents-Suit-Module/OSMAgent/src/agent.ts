...
const provideAgentHandler = (oracleAddresses: string[]): HandleTransaction => {
  const bigDeviationNextPrice: HandleTransaction = provideBigQueuedPriceDeviationHandler(
    oracleAddresses
  );
  ...
  return async (txEvent: TransactionEvent): Promise<Finding[]> => {
    ...
    findings = findings.concat(await bigDeviationNextPrice(txEvent));
...
export default {
  handleTransaction: provideAgentHandler(ORACLE_ADDRESSES)
};
