...
import { zeroAddress, keccakFromString, bufferToHex, addHexPrefix, stripHexPrefix } from "ethereumjs-util";
...
const buildLogForOwnershipTransferenceEvent = (from: string, to: string) : Log => {
    const topics : string[] = [
...
describe("trasnferred ownership agent", () => {
  ...
  describe("handleTransaction", () => {
    ...
    it("Returns empty findings if there is no OwnershipTransferred event from a non zero address", async () => {
        const log: Log = buildLogForOwnershipTransferenceEvent(zeroAddress(), testAddress1); 
