...
export function createFinding(feeFrom: "FlashLoan" | "Swap", newFee: ethers.BigNumber): Finding {
  return Finding.from({
    name: `${feeFrom} fee changed from the Balancer 'ProtocolFeesCollector' contract`,
    description: `${feeFrom} fee percentage has changed to ${ethers.utils.formatUnits(newFee, 16)}`,
