...
const genFinding = (name: string, description: string): Finding =>
  Finding.fromObject({
    name,
    description,
...
export const EVENTS: Event[] = [
  {
    ...
    createFinding: (addr: string): Finding =>
      genFinding(
  ...
  },
  {
    ...
    createFinding: (threshold: string): Finding =>
      genFinding(
...
const handleTransaction: HandleTransaction = async (
  ...
  EVENTS.forEach((e: Event) => {
    txEvent.filterEvent(e.signature).forEach((log: Log) => {
      findings.push(e.createFinding(log.data));
    });
  });
