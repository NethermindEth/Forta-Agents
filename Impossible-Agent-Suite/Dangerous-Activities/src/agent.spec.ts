...
describe("Dangerous Activities monitor tests suite", () => {
  ...
  const buildHandler = (list0: string[], list1: string[]) => provideHandleTransaction(
    utils.listVerifier(toAddresses(list0)),
    utils.listVerifier(toAddresses(list1)),
  );

  it("should detect dangerous interaction", async () => {
    const handler: HandleTransaction = buildHandler(
      ["0xf00d", "0xdead"],
      ["0xbad", "0xfee", "0xb4d", "0xf33"],
    );

    const tx: TransactionEvent = new TestTransactionEvent()
      .addInvolvedAddresses(...toAddresses([
        "0xf00d", "0xdead", "0xda0", "0xfee", "0xb4d",
      ]))
